...
use crate::connection::{AnsiAsyncTransactionManager, AsyncConnection, AsyncTransactionManager};
use crate::pg::Pg;
...
where
    C: AsyncConnection<Backend = Pg, TransactionManager = AnsiAsyncTransactionManager>,
{
    ...
    where
        ...
        T: 'b,
        E: From<crate::result::Error> + 'b,
    {
        let mut query_builder = <Pg as Backend>::QueryBuilder::default();
        self.to_sql(&mut query_builder, &Pg)?;
        let sql = query_builder.finish();

        AnsiAsyncTransactionManager::begin_transaction_sql(&mut *self.connection, &sql).await?;
        match f(&mut *self.connection).await {
            Ok(value) => {
                AnsiAsyncTransactionManager::commit_transaction(&mut *self.connection).await?;
                Ok(value)
            }
            Err(e) => {
                AnsiAsyncTransactionManager::rollback_transaction(&mut *self.connection).await?;
                Err(e)
            }
        }
    }
