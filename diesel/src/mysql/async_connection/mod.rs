//! An async MySQL connection, over `mysql_async`.
//!
//! Moved here from the `diesel-async` crate when that was folded in; it is a
//! submodule of `mysql` so that everything speaking this backend is reachable
//! from one path.
...
use crate::mysql::{Mysql, MysqlQueryBuilder, MysqlType};
use crate::query_builder::QueryBuilder;
use crate::query_builder::{bind_collector::RawBytesBindCollector, QueryFragment, QueryId};
...
pub struct AsyncMysqlConnection {
    ...
    transaction_manager: AnsiAsyncTransactionManager,
...
impl AsyncConnection for AsyncMysqlConnection {
    ...
    async fn establish(database_url: &str) -> crate::ConnectionResult<Self> {
...
fn update_transaction_manager_status<T>(
    ...
    if let Err(crate::result::Error::DatabaseError(
...
fn prepare_statement_helper<'a>(
    ...
    sql: &str,
    _is_for_cache: crate::connection::statement_cache::PrepareForCache,
    _metadata: &[MysqlType],
...
impl AsyncMysqlConnection {
    ...
    {
        self.instrumentation()
            .on_connection_event(InstrumentationEvent::start_query(&crate::debug_query(
                &query,
            )));
    ...
    async fn poll_result_stream<'conn>(
        ...
        let stmt_for_exec = match stmt {
            ...
            // No wildcard arm. `MaybeCached` is `#[non_exhaustive]`, which
            // forced one on this match while the code lived in another
            // crate — and made "Diesel has only two variants here at the
            // time of writing" an assumption checked at run time, in a
            // panic message pointing at an issue tracker. In this crate
            // the attribute does not apply, so the match is exhaustive and
            // a third variant is a compile error at exactly this line.
        };
        ...
        let stream = res
            ...
            .map_err(ErrorHelper)?
            .ok_or_else(|| {
                crate::result::Error::DeserializationError(Box::new(
                    crate::result::UnexpectedEndOfRow,
                ))
            })?
    ...
    async fn establish_connection_inner(
        ...
        let opts = Opts::from_url(database_url)
            .map_err(|e| crate::result::ConnectionError::InvalidConnectionUrl(e.to_string()))?;
