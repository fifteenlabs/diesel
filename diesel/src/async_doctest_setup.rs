...
cfg_if::cfg_if! {
    if #[cfg(feature = "async-postgres")] {
        ...
        async fn create_tables(connection: &mut AsyncPgConnection) {
            ...
            crate::sql_query("CREATE TEMPORARY TABLE users (
            ...
            crate::sql_query("CREATE TEMPORARY TABLE comments (
                id SERIAL PRIMARY KEY,
                post_id INTEGER NOT NULL,
                body VARCHAR NOT NULL
            )").execute(connection).await.unwrap();
            crate::sql_query("INSERT INTO comments (post_id, body) VALUES
                (1, 'Great post'),
                (2, 'Yay! I am learning Rust'),
                (3, 'I enjoyed your post')").execute(connection).await.unwrap();

            crate::sql_query("CREATE TEMPORARY TABLE brands (
                id SERIAL PRIMARY KEY,
    ...
    }  else if #[cfg(feature = "async-mysql")] {
        ...
        async fn create_tables(connection: &mut AsyncMysqlConnection) {
            use diesel_async::RunQueryDsl;
            use diesel_async::AsyncConnection;
            crate::sql_query("CREATE TEMPORARY TABLE IF NOT EXISTS users (
                id INTEGER PRIMARY KEY AUTO_INCREMENT,
                name TEXT NOT NULL
            ) CHARACTER SET utf8mb4").execute(connection).await.unwrap();


            crate::sql_query("CREATE TEMPORARY TABLE IF NOT EXISTS animals (
                id INTEGER PRIMARY KEY AUTO_INCREMENT,
            ...
            crate::sql_query("CREATE TEMPORARY TABLE IF NOT EXISTS posts (
                id INTEGER PRIMARY KEY AUTO_INCREMENT,
                user_id INTEGER NOT NULL,
                title TEXT NOT NULL
            ) CHARACTER SET utf8mb4").execute(connection).await.unwrap();

            crate::sql_query("CREATE TEMPORARY TABLE IF NOT EXISTS comments (
                id INTEGER PRIMARY KEY AUTO_INCREMENT,
                post_id INTEGER NOT NULL,
                body TEXT NOT NULL
            ) CHARACTER SET utf8mb4").execute(connection).await.unwrap();
            crate::sql_query("CREATE TEMPORARY TABLE IF NOT EXISTS brands (
                id INTEGER PRIMARY KEY AUTO_INCREMENT,
            ...
            connection.begin_test_transaction().await.unwrap();
            crate::sql_query("INSERT INTO users (name) VALUES ('Sean'), ('Tess')").execute(connection).await.unwrap();
            crate::sql_query("INSERT INTO posts (user_id, title) VALUES
                (1, 'My first post'),
                (1, 'About Rust'),
                (2, 'My first post too')").execute(connection).await.unwrap();
            crate::sql_query("INSERT INTO comments (post_id, body) VALUES
                ...
                (2, 'Yay! I am learning Rust'),
                (3, 'I enjoyed your post')").execute(connection).await.unwrap();
            crate::sql_query("INSERT INTO animals (species, legs, name) VALUES
                               ('dog', 4, 'Jack'),
    ...
    }  else if #[cfg(feature = "async-sqlite")] {
        use diesel_async::sync_connection_wrapper::SyncConnectionWrapper;
        use crate::sqlite::SqliteConnection;
        #[allow(dead_code)]
        type DB = crate::sqlite::Sqlite;
        #[allow(dead_code)]
