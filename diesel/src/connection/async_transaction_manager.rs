...
pub trait AsyncTransactionManager<Conn: AsyncConnection>: Send {
...
/// An implementation of `AsyncTransactionManager` which can be used for backends
...
pub struct AnsiAsyncTransactionManager {
    ...
    // into `AnsiAsyncTransactionManager::critical_transaction_block`
    // below
    //
...
impl AnsiAsyncTransactionManager {
...
impl<Conn> AsyncTransactionManager<Conn> for AnsiAsyncTransactionManager
where
    Conn: AsyncConnection<TransactionManager = Self>,
{
    ...
    async fn begin_transaction(conn: &mut Conn) -> QueryResult<()> {
        ...
        conn.instrumentation()
            .on_connection_event(InstrumentationEvent::BeginTransaction { depth });
