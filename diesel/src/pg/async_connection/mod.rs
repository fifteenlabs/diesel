...
use crate::connection::async_stmt_cache::{CallbackHelper, QueryFragmentHelper};
use crate::connection::statement_cache::{
    PrepareForCache, QueryFragmentForCachedStatement, StatementCache,
};
use crate::connection::StrQueryHelper;
use crate::connection::{
    AnsiAsyncTransactionManager, AsyncConnection, AsyncConnectionCore, SimpleAsyncConnection,
};
use crate::connection::{CacheSize, Instrumentation};
use crate::connection::{DynInstrumentation, InstrumentationEvent};
use crate::pg::{
...
use crate::{ConnectionError, ConnectionResult, QueryResult};
...
pub struct AsyncPgConnection {
    ...
    stmt_cache: Mutex<StatementCache<crate::pg::Pg, Statement>>,
    transaction_state: Mutex<AnsiAsyncTransactionManager>,
    metadata_cache: Mutex<PgMetadataCache>,
    connection_future: Option<broadcast::Receiver<Arc<tokio_postgres::Error>>>,
    notification_rx: Option<mpsc::UnboundedReceiver<QueryResult<crate::pg::PgNotification>>>,
...
impl AsyncConnection for AsyncPgConnection {
    ...
    fn transaction_state(&mut self) -> &mut AnsiAsyncTransactionManager {
...
async fn load_prepared(
    ...
    Ok(res
        .map_err(|e| crate::result::Error::from(ErrorHelper(e)))
        .map_ok(PgRow::new)
        .boxed())
}
...
fn update_transaction_manager_status<T>(
    ...
    transaction_manager: &mut AnsiAsyncTransactionManager,
) -> QueryResult<T> {
    if let Err(crate::result::Error::DatabaseError(DatabaseErrorKind::SerializationFailure, _)) =
        query_result
    {
...
fn type_from_oid(t: &PgTypeMetadata) -> QueryResult<Type> {
    let oid = t
        .oid()
        .map_err(|e| crate::result::Error::SerializationError(Box::new(e) as _))?;
...
impl AsyncPgConnection {
    ...
    async fn setup(
        ...
        let mut conn = Self {
            ...
            transaction_state: Mutex::new(AnsiAsyncTransactionManager::default()),
    ...
    async fn set_config_options(&mut self) -> QueryResult<()> {
        use crate::async_dsl::RunQueryDsl;

        futures_util::future::try_join(
            crate::sql_query("SET TIME ZONE 'UTC'").execute(&mut &*self),
            crate::sql_query("SET CLIENT_ENCODING TO 'UTF8'").execute(&mut &*self),
        )
        .await?;
        Ok(())
    }
    ...
    pub fn notifications_stream(
        &mut self,
    ) -> impl futures_core::Stream<Item = QueryResult<crate::pg::PgNotification>> + '_ {
...
fn construct_bind_data(query: &dyn QueryFragment<crate::pg::Pg>) -> BindData {
    ...
    let mut bind_collector_0 = RawBytesBindCollector::<crate::pg::Pg>::new();
    ...
    if metadata_lookup_0.custom_oid {
        ...
        let mut max_oid = bind_collector_0
            ...
            .max()
            .unwrap_or_default();
        let mut bind_collector_1 = RawBytesBindCollector::<crate::pg::Pg>::new();
...
async fn drive_future<R>(
    connection_future: Option<broadcast::Receiver<Arc<tokio_postgres::Error>>>,
    client_future: impl Future<Output = Result<R, crate::result::Error>>,
) -> Result<R, crate::result::Error> {
    if let Some(mut connection_future) = connection_future {
        ...
        match futures_util::future::select(client_future, connection_future).await {
            ...
            Either::Right((Err(e), _)) => Err(crate::result::Error::DatabaseError(
...
fn drive_connection<S>(
    ...
    mpsc::UnboundedReceiver<QueryResult<crate::pg::PgNotification>>,
    oneshot::Sender<()>,
)
where
    S: tokio_postgres::tls::TlsStream + Unpin + Send + 'static,
        ...
        loop {
            match futures_util::future::select(&mut shutdown_rx, conn.next()).await {
                ...
                Either::Right((Some(Ok(tokio_postgres::AsyncMessage::Notification(notif))), _)) => {
                    let _: Result<_, _> = notification_tx.send(Ok(crate::pg::PgNotification {
...
mod tests {
    ...
    async fn pipelining() {
        ...
        let conn = crate::pg::AsyncPgConnection::establish(&database_url)
            .await
            .unwrap();

        let q1 = crate::select(1_i32.into_sql::<Integer>());
        let q2 = crate::select(2_i32.into_sql::<Integer>());
    ...
    async fn pipelining_with_composed_futures() {
        ...
        async fn fn12(mut conn: &AsyncPgConnection) -> QueryResult<(i32, i32)> {
            let f1 = crate::select(1_i32.into_sql::<Integer>()).get_result::<i32>(&mut conn);
            let f2 = crate::select(2_i32.into_sql::<Integer>()).get_result::<i32>(&mut conn);

            try_join(f1, f2).await
        }

        async fn fn34(mut conn: &AsyncPgConnection) -> QueryResult<(i32, i32)> {
            let f3 = crate::select(3_i32.into_sql::<Integer>()).get_result::<i32>(&mut conn);
            let f4 = crate::select(4_i32.into_sql::<Integer>()).get_result::<i32>(&mut conn);
    ...
    async fn pipelining_with_composed_futures_and_transaction() {
        ...
        async fn fn37(
            ...
            let f3 = crate::select(0_i32.into_sql::<Integer>()).execute(&mut conn);
            let f4 = crate::select(4_i32.into_sql::<Integer>()).load::<i32>(&mut conn);
            let f5 = crate::select(5_i32.into_sql::<Integer>()).get_result::<i32>(&mut conn);
            let f6 = crate::select(6_i32.into_sql::<Integer>()).get_results::<i32>(&mut conn);
            let f7 = crate::select(7_i32.into_sql::<Integer>()).first::<i32>(&mut conn);
