...
//! [`crate::connection::AsyncConnection`]. Using this type
...
/// A helper type that wraps an [`AsyncConnection`][crate::connection::AsyncConnection] to
...
/// A helper type that wraps an [`crate::connection::AsyncConnectionWrapper`] to
/// provide a sync [`diesel::Connection`] implementation.
///
/// Internally this wrapper type will use `block_on` to wait for
...
pub(crate) mod implementation {
    use crate::connection::{CacheSize, Instrumentation, SimpleConnection};
    use std::ops::{Deref, DerefMut};

    use super::*;

    // Wraps a live connection and a runtime; neither has a `Debug` worth
    // forwarding, and deriving one would print the wrapper and nothing else.
    ...
    where
        C: crate::connection::AsyncConnection,
        B: BlockOn + Send,
    {
    ...
    impl<C, B> crate::connection::SimpleConnection for AsyncConnectionWrapper<C, B>
    where
        C: crate::connection::SimpleAsyncConnection,
        B: BlockOn,
    {
        fn batch_execute(&mut self, query: &str) -> crate::QueryResult<()> {
            let f = self.inner.batch_execute(query);
            self.runtime.block_on(f)
        }
    }

    impl<C, B> crate::connection::ConnectionSealed for AsyncConnectionWrapper<C, B> {}

    impl<C, B> crate::connection::Connection for AsyncConnectionWrapper<C, B>
    where
        C: crate::connection::AsyncConnection,
        B: BlockOn + Send,
    {
        ...
        fn establish(database_url: &str) -> crate::ConnectionResult<Self> {
        ...
        fn execute_returning_count<T>(&mut self, source: &T) -> crate::QueryResult<usize>
        where
            T: crate::query_builder::QueryFragment<Self::Backend> + crate::query_builder::QueryId,
        {
        ...
        fn transaction_state(
            &mut self,
        ) -> &mut <Self::TransactionManager as crate::connection::TransactionManager<Self>>::TransactionStateData{
    ...
    impl<C, B> crate::connection::LoadConnection for AsyncConnectionWrapper<C, B>
    where
        C: crate::connection::AsyncConnection,
        B: BlockOn + Send,
    {
        ...
        fn load<'conn, 'query, T>(
            &'conn mut self,
            source: T,
        ) -> crate::QueryResult<Self::Cursor<'conn, 'query>>
        where
            T: crate::query_builder::Query
                + crate::query_builder::QueryFragment<Self::Backend>
                + crate::query_builder::QueryId
                + 'query,
            Self::Backend: crate::expression::QueryMetadata<T::SqlType>,
        {
    ...
    impl<C, B> crate::connection::TransactionManager<AsyncConnectionWrapper<C, B>>
        ...
        B: BlockOn + Send,
    {
        type TransactionStateData =
            <C::TransactionManager as crate::connection::AsyncTransactionManager<C>>::TransactionStateData;

        fn begin_transaction(conn: &mut AsyncConnectionWrapper<C, B>) -> crate::QueryResult<()> {
            let f = <C::TransactionManager as crate::connection::AsyncTransactionManager<_>>::begin_transaction(
                &mut conn.inner,
            );
            conn.runtime.block_on(f)
        }

        fn rollback_transaction(conn: &mut AsyncConnectionWrapper<C, B>) -> crate::QueryResult<()> {
            let f = <C::TransactionManager as crate::connection::AsyncTransactionManager<_>>::rollback_transaction(
                &mut conn.inner,
            );
            conn.runtime.block_on(f)
        }

        fn commit_transaction(conn: &mut AsyncConnectionWrapper<C, B>) -> crate::QueryResult<()> {
            let f = <C::TransactionManager as crate::connection::AsyncTransactionManager<_>>::commit_transaction(
                &mut conn.inner,
            );
            conn.runtime.block_on(f)
        }
        ...
        fn transaction_manager_status_mut(
            ...
            <C::TransactionManager as crate::connection::AsyncTransactionManager<_>>::transaction_manager_status_mut(
                &mut conn.inner,
            )
        }

        fn is_broken_transaction_manager(conn: &mut AsyncConnectionWrapper<C, B>) -> bool {
            <C::TransactionManager as crate::connection::AsyncTransactionManager<_>>::is_broken_transaction_manager(
                &mut conn.inner,
            )
        }
    }

    // Both halves are needed and they are different features: `r2d2` is this
    // crate's *sync* pool, which supplies the trait being implemented, while
    // `async-pool` supplies `PoolableConnection`, which the body calls. In
    // `diesel-async` one feature named `r2d2` implied the other, because that
    // crate's `r2d2 = ["pool", "diesel/r2d2"]`. Here `r2d2` cannot imply
    // `async-pool` — a sync-only r2d2 user must not be made to compile the
    // futures stack — so the `all(…)` says out loud what used to be implied.
    #[cfg(all(feature = "r2d2", feature = "async-pool"))]
    impl<C, B> crate::r2d2::R2D2Connection for AsyncConnectionWrapper<C, B>
    where
        B: BlockOn,
        Self: crate::Connection,
        C: crate::connection::AsyncConnection<Backend = <Self as crate::Connection>::Backend>
        ...
        crate::query_builder::SqlQuery:
            crate::query_dsl::async_run_query_dsl::methods::ExecuteDsl<C>,
    {
        fn ping(&mut self) -> crate::QueryResult<()> {
    ...
    impl<C, B> crate::migration::MigrationConnection for AsyncConnectionWrapper<C, B>
    where
        B: BlockOn,
        Self: crate::Connection,
    {
        fn setup(&mut self) -> crate::QueryResult<usize> {
            self.batch_execute(crate::migration::CREATE_MIGRATIONS_TABLE)
                .map(|()| 0)
        }
    }
