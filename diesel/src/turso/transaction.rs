...
use crate::connection::{AsyncConnection, AsyncTransactionManager};
...
/// Turso's [`AsyncTransactionManager`](crate::connection::AsyncTransactionManager).
///
/// Ordinary ANSI nesting — `BEGIN`, then `SAVEPOINT` per depth — with one
/// addition: see [`Self::poisoned`]'s field comment for the case a nested
/// rollback cannot express in SQL alone.
...
impl<Conn> AsyncTransactionManager<Conn> for TursoTransactionManager
where
    Conn: AsyncConnection<TransactionManager = Self>,
{
    ...
    async fn begin_transaction(conn: &mut Conn) -> QueryResult<()> {
        ...
        conn.instrumentation()
            .on_connection_event(InstrumentationEvent::BeginTransaction { depth: new_depth });
