...
pub mod multiconnection {
    ...
    // Exposed for out-of-tree backends that want to reimplement the
    // SQLite-style UPSERT specialization (`OnConflictSelectWrapper` +
    // `WhereClause` / `BoxedWhereClause`). Without these, external
