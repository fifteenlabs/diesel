...
pub enum PoolError {
    ...
    ConnectionError(crate::result::ConnectionError),
...
pub trait PoolableConnection: AsyncConnection {
    ...
    ) -> impl Future<Output = crate::QueryResult<()>> + Send
    where
        for<'a> Self: 'a,
        crate::dsl::select<crate::dsl::AsExprOf<i32, crate::sql_types::Integer>>:
            crate::query_dsl::async_run_query_dsl::methods::ExecuteDsl<Self>,
        ...
        async move {
            match config {
                ...
                RecyclingMethod::Verified => {
                    crate::select(1_i32.into_sql::<crate::sql_types::Integer>())
                        .execute(self)
                        .await
                        .map(|_| ())
                }
                RecyclingMethod::CustomQuery(query) => crate::sql_query(query.as_ref())
                    .execute(self)
                    .await
                    .map(|_| ()),
